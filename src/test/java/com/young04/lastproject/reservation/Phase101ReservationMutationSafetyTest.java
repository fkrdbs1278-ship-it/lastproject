...
class Phase101ReservationMutationSafetyTest {
    ...
    @Mock AvailableTimeService availableTimeService;
    @Mock ServiceMenuReader serviceMenuReader;
    @Mock HairStyleReader hairStyleReader;
    @Mock BusinessHourRepository businessHourRepository;
    @Mock ReservationNotificationPublisher notificationPublisher;
    @Mock NoShowRepository noShowRepository;
    ...
    void 예약시작전에는_시술완료로_변경할수없다() {
        Reservation reservation = guestReservation(
                LocalDateTime.now().plusDays(1)
        );
        reservation.confirm();

        when(reservationRepository.findByIdForUpdate(400L))
                .thenReturn(Optional.of(reservation));

        assertThatThrownBy(() ->
                reservationService.completeReservation(400L)
        )
                .isInstanceOf(InvalidReservationStatusException.class)
                .hasMessageContaining("예약 시작 전");
    ...
    void 지난예약은_고객이_직접취소할수없다() {
        ...
        when(reservationRepository.findByIdForUpdate(401L))
                .thenReturn(Optional.of(reservation));
    ...
    void 예약시작전에는_노쇼처리할수없다() {
        ...
        when(reservationRepository.findByIdForUpdate(402L))
                .thenReturn(Optional.of(reservation));
    ...
    private Reservation guestReservation(
        ...
        return Reservation.createGuestReservation(
                "테스트고객",
                "01012345678",
                1L,
                "커트",
